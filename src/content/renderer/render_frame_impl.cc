...
bool RenderFrameImpl::OnMessageReceived(const IPC::Message& msg) {
    //ChromePic
  /*
  if (IPC_MESSAGE_ID_CLASS(msg.type()) == 3 &&  IPC_MESSAGE_ID_LINE(msg.type()) == 192)
  {
      fprintf(stderr, "RenderFrameImpl - OnMessageReceived!: %ld\n", base::TimeTicks::Now().ToInternalValue());
    const void* frame_id = static_cast<const void*>(this);
    std::stringstream ss;
    ss <<frame_id; 
    std::string frame_id_string = ss.str();
    //std::cout<<  "Dispatch message: listener id is: " << listener_id;
    fprintf(stderr, "RenderFrameImpl:: OnMessageReceived: frame id is: %s\n", frame_id_string.c_str());
      //print_stacktrace();
  }
  */
    //ChromePic
...
void RenderFrameImpl::OnSerializeAsMHTML(
  ...
  log_stream << "RenderFrameImpl: Begin OnSerializeAsMHTML " << "Job ID: " << params.job_id;
  ...
  Send(new FrameHostMsg_SerializeAsMHTMLResponse(
      ...
      digests_of_uris_of_serialized_resources));
  log_stream << "RenderFrameImpl: Done with OnSerializeAsMHTML " << "Job ID: " << params.job_id;
  Logger::LogLineScreen(log_stream.str(), true);
  log_stream.str("");
}
