...
bool MHTMLGenerationManager::Job::SendToNextRenderFrame() {
  //fprintf(stderr, "========== SendToNextRenderFrame()===========%ld, job_id_: %d\n", base::TimeTicks::Now().ToInternalValue(), job_id_);
  ...
  log_stream << "MHTMLGenerationManager::Job::SendToNextRenderFrame: IPC Message Sent";
  Logger::LogLineScreen(log_stream.str(), true);
  
  //IPC::Message* msg = new InputMsg_ChromePic(rfh->GetRoutingID());
  //fprintf(stderr, "the codes for input msg: %d, %d \n", IPC_MESSAGE_ID_CLASS(msg->type()), IPC_MESSAGE_ID_LINE(msg->type()));
  //rfh->Send(msg);
  //IPC::Message* msg2 = new FrameMsg_SerializeAsMHTML(rfh->GetRoutingID(), ipc_params);
  //fprintf(stderr, "the codes for frame msg: %d, %d \n", IPC_MESSAGE_ID_CLASS(msg2->type()), IPC_MESSAGE_ID_LINE(msg2->type()));
  //rfh->Send(msg2);
  //fprintf(stderr, "==========Finished SendToNextRenderFrame()===========%ld, job_id_: %d\n", base::TimeTicks::Now().ToInternalValue(), job_id_);
  return true;
}
...
void MHTMLGenerationManager::Job::AddFrame(RenderFrameHost* render_frame_host) {
  //fprintf(stderr, "Calling AddFrame!! ==%ld, job_id_: %d\n", base::TimeTicks::Now().ToInternalValue(), job_id_);
...
void MHTMLGenerationManager::Job::CloseFile(
  ...
  //fprintf(stderr, "Job done!! Job done!!\n");
  //ChromePic
  //TODO: Change this!!
  //Close file from the main thread instead of doing nothing!
  callback.Run(1);
  return;
  //ChromePic
...
bool MHTMLGenerationManager::Job::OnSerializeAsMHTMLResponse(
    RenderFrameHostImpl* sender,
    const std::set<std::string>& digests_of_uris_of_serialized_resources) {
  //fprintf(stderr, "MHTMLGenerationManager:: OnSerializeAsMHTMLResponse!!\n");
  ...
  //fprintf(stderr, "Calling SendToNextRenderFrame!!\n");
...
void MHTMLGenerationManager::SaveMHTML(WebContents* web_contents,
  ...
  //ChromePic
  //Creating file on the main thread instead of the browser thread.
  /*
  BrowserThread::PostTaskAndReplyWithResult(
      BrowserThread::FILE, FROM_HERE,
      ...
      base::Bind(&MHTMLGenerationManager::OnFileAvailable,
                 ...
                 job_id));
 */
  OnFileAvailable(job_id, CreateFile(file_path));
 //ChromePic
}
...
base::File MHTMLGenerationManager::CreateFile(const base::FilePath& file_path) {
  //ChromePic
  //As CreateFile is made to call by the main thread itself this check is being commented out
  //DCHECK_CURRENTLY_ON(BrowserThread::FILE);
  //ChromePic
