...
InputRouterImpl::InputRouterImpl(IPC::Sender* sender,
  ...
  //ChromePic
  snapshot_handler_ = new SnapshotHandler(sender, client, routing_id);
  //ChromePic
}

InputRouterImpl::~InputRouterImpl() {
  //ChromePic
  snapshot_handler_->logger_->Flush();
  //ChromePic
...
bool InputRouterImpl::OfferToRenderer(const WebInputEvent& input_event,
  ...
  //ChromePic
  snapshot_handler_->HandleInputEvent(*event_to_send, latency_info);
  
  //We always sending true even for failures, not sure what this will affect?
