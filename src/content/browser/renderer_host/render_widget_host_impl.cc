...
//ChromePic
void RenderWidgetHostImpl::CopyFromBackingStoreProxy(
    ...
    CopyFromBackingStore(
        src_subrect, accelerated_dst_size, callback, preferred_color_type);
}

void RenderWidgetHostImpl::TakeDOMSnapshot(
    const base::FilePath& path,
    const base::Callback<void(int64_t)>& callback){
    RenderViewHost* rvh = RenderViewHost::From(this);
    if (!rvh) {
        fprintf(stderr, "RWHI: Error, no RenderViewHost available\n");
        RenderFrameHost* rfh = RenderFrameHost::FromID(routing_id_, GetProcess()->GetID());
        if (!rfh)
            fprintf(stderr, "RWHI: Error, no RenderFrameHost available either!!\n");
    }
    WebContents* web_contents = WebContents::FromRenderViewHost(rvh);
    if (!web_contents) {
        fprintf(stderr, "RWHI: Error, no WebContents available\n");
    }
    web_contents->GenerateMHTML(path, callback);
}
//ChromePic
