...
class GFX_EXPORT Size {
 public:
  //ChromePic
  Size() :  process_id(-1), routing_id(-1), snapshot_id(-1), width_(0), height_(0) {}
  Size(int width, int height)
      :process_id(-1), routing_id(-1), snapshot_id(-1), width_(width < 0 ? 0 : width), height_(height < 0 ? 0 : height) {}
  //ChromePic
